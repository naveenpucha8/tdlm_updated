...
class TopicModel:
    def __init__(self, is_training, vocab_size, batch_size, num_steps, num_classes, cf):
        ...
        self.y = torch.IntTensor(num_steps)
        self.tm_mask = torch.FloatTensor(num_steps)
        self.tag = torch.IntTensor(cf.tag_len)
        ...
        if is_training and cf.num_samples > 0:
        ...
        self.tm_cost = torch.autograd.Variable(torch.FloatTensor(1).zero_())
        # self.tm_train_op = 0

    def varFunc(self, y, m, d, t):
        self.doc = d
        self.y = y
        self.tm_mask = m
        self.tag = t
        doc_inputs = []
        for i in self.doc:
        	doc_inputs.append(self.conv_word_embedding[i])
        	doc_inputs = torch.from_numpy(np.array(doc_inputs))
        ...
        if not (is_training and cf.num_samples > 0):
			...
			logp = F.log_softmax(self.tm_logits)
			tm_crossent = (torch.gather(logp,dim=1,self.y.view(-1))).view(-1)
	...
	def get_topics(self, topn):
		topics = []
		entropy = []
		tw_dist = torch.nn.SoftMax(self.topic_output_embedding.mm(self.tm_softmax_w) + self.tm_softmax_b)
